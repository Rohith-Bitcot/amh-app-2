...
function MiniTable({
  ...
  return (
    <div className="bg-white rounded-2xl shadow-[0px_4px_15px_0px_rgba(0,0,0,0.10)] p-4">
        ...
        <thead>
          <tr className="bg-gradient-to-r from-sky-700 to-sky-500">
            {columns.map((col) => (
              <th
                key={col}
                className="px-3 py-2 text-white text-xs font-medium font-heading text-left"
              >
                {col}
              </th>
            ))}
          ...
          {rows.map((row, idx) => (
            ...
            >
              {columns.map((col) => (
                <td
                  key={col}
                  className="px-3 py-2 text-sm font-heading text-neutral-700"
                >
                  {row[col]}
                </td>
              ))}
...
export default function FunnelJourney() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gradient-to-r from-[#004F86] to-[#123E5D] rounded-2xl p-6">
        <TabGroup
          tabs={[
