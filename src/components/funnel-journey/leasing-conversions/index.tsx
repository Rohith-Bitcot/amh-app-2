...
<tr className="text-white">
  {mainHeaders.map((header, idx) => (
    <th
      ...
      className={`px-3 py-2 text-sm font-semibold font-heading text-center border-0 border-r border-white/10
      ...
      <div className={`flex items-center justify-center gap-2 ${header.hasFilter ? "" : "w-full"}`}>
        ...
        {header.hasFilter && <FilterIcon className="w-4 h-4" />}
                            ...
                            <tr className="bg-white">
                                {/* District under Geo */}
                                <th className="px-4 py-2 text-[#1F1F22] text-[14px] font-medium font-heading text-center border-b border-r border-neutral-200 bg-[#F0F8FE]" style={{
                                    fontWeight:500
                                }}>
                                    District
                                </th>
                                {/* Unique Showing sub-columns */}
                                <th className="px-3 py-2 text-[#1F1F22] text-[14px] uppercase font-medium font-heading text-center border-b border-r border-neutral-200 bg-[#F0F8FE]" style={{
                                    fontWeight:500
                                }}>
                                    T7
                                </th>
                                ...
                                <th className="px-3 py-2 text-[#1F1F22] text-[14px] uppercase font-medium font-heading text-center border-b border-neutral-200 bg-[#F0F8FE]">
                                    PW
                                </th>
                                <th className="px-3 py-2 text-[#1F1F22] text-[14px] uppercase font-medium font-heading text-center bg-[#F0F8FE]">
                                    PY
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            {leasingConversionsData.rows.map((row, rowIndex) => {
                                const isLastRow = rowIndex === leasingConversionsData.rows.length - 1;
                                const borderB = isLastRow ? "border-b-0" : "border-b border-neutral-100";
                                return (
                                <tr
                                    key={row.geo}
                                    className={cn(
                                ...
                                >
                                    ...
                                    {row.uniqueShowing.map((val, vi) => (
                                        <td
                                            ...
                                            className={`px-3 py-3 text-sm font-normal font-heading text-neutral-800 text-center border-r border-neutral-100 ${borderB}`}
                                        >
                                            {val}
                                        </td>
                                    ))}
                                    ...
                                    {row.appStartSubmit.map((val, vi) => (
                                        <td
                                            ...
                                            className={`px-3 py-3 text-sm font-normal font-heading text-neutral-800 text-center bg-[#F0F8FE] border-r border-neutral-100 ${borderB}`}
                                        >
                                            {val}
                                        </td>
                                    ))}
